...
############## CREATE ################
 ...
 post '/add/Category' do
    ...
    params["CategoryNum"] ? (CategoryNum = params["CategoryNum"]) : (halt 400, {"message": "Missing Paramaters"}.to_json)
    params["Name"] ? (Name = params["Name"]) : (halt 400, {"message": "Missing Paramaters"}.to_json)
    params["ReqHours"] ? (ReqHours = params["ReqHours"]) : (halt 400, {"message": "Missing Paramaters"}.to_json)

    if CategoryNum != '' && ReqHours != '' && Name != '' 
      c = Categories.new
      ...
      halt 200, {"message": "Successfully added Category to Categories Table"}.to_json
    else
      halt 400, {"message": "Missing Paramaters"}.to_json
    end
  ...
  post 'add/prerequisites' do 
  	api_authenticate!
  	if !current_user.admin
  		halt 401, {"message" : "Unauthorized user"}.to_json
  	end
  	params["CourseID"] ? (CourseID = params["CourseID"] : (halt 400, {"message": "Missing Parameters"}.to_json)
	params["PreReqID"] ? (PreReqID = params["PreReqID"] : (halt 400, {"message": "Missing Parameters"}.to_json)
  ...
  post 'add/CourseAlt' do 
  	api_authenticate!
  	if !current_user.admin
  		halt 401, {"message" : "Unauthorized user"}.to_json
  	end
  	params["CourseID"] ? (CourseID = params["CourseID"] : (halt 400, {"message": "Missing Parameters"}.to_json)
	params["AltID"] ? (AltID = params["AltID"] : (halt 400, {"message": "Missing Parameters"}.to_json)
